...
if activate:
    # Least activated neurons in s1 
    top_indices = torch.topk(-z1_signed, k=num_selected).indices  # highest in s1 - s2
    neuron_list = top_indices
else:
    # Most activated neurons in s1
    top_indices = torch.topk(s1_acts, k=num_selected).indices  # highest in s2 - s1
    neuron_list = s1[top_indices]
...
for n_id in neuron_list:
    ...
    for i in range(num_iters):
        ...
        ranks.append(best_rank if type(best_rank)==int else best_rank.item())
