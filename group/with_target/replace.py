...
h1 = model(x1_raw_processed, output_hidden_states=True).hidden_states[layer_num + 1][0][-1]
z1 = sae.pre_acts(h1)
...
print(f"Initial overlap = {count_common(s1, s2) / len(s2)}")
...
for t in range(1, x1_raw.shape[-1]):
    ...
    for i in range(num_iters):
        ...
        best_loss = cos_sim(z1_batch[best_idx], z2) 
