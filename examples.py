...
def count_common(x, y):
    ...
    for elem in x:
        if  elem in y:
            num_common += 1
    return num_common, num_common / len(y)

layer_num = 20
# sae = Sae.load_many_from_hub("EleutherAI/sae-llama-3-8b-32x")
sae = Sae.load_from_disk(BASE_DIR + f"layers.{layer_num}").to(DEVICE)

tokenizer = AutoTokenizer.from_pretrained("meta-llama/Meta-Llama-3-8B", cache_dir="/n/holyscratch01/hlakkaraju_lab/Lab/aaronli/models")
...
seq_0 = tokenizer("The cat slept peacefully on the sunny windowsill", return_tensors="pt").to(DEVICE)
seq_1 = tokenizer("The cat slept peacefully on the rainy windowsill", return_tensors="pt").to(DEVICE)
out_tokens_0 = model.generate(
    seq_0['input_ids'],
...
# Decode the output
generation_0 = tokenizer.decode(out_tokens_0[0], skip_special_tokens=True)
print(generation_0)
generation_1 = tokenizer.decode(out_tokens_1[0], skip_special_tokens=True)
print(generation_1)
output_0 = model(**seq_0, output_hidden_states=True)
output_1 = model(**seq_1, output_hidden_states=True)
h_0 = output_0.hidden_states[layer_num+1][0][-1] # (4096,)
z_0 = sae.pre_acts(h_0)
h_1 = output_1.hidden_states[layer_num+1][0][-1] # (131072,)
z_1 = sae.pre_acts(h_1)
# print(h_0.shape, z_0.shape, h_1.shape, z_1.shape)
latent_0, latent_1 = sae.encode(h_0), sae.encode(h_1)
top_0, top_1 = latent_0.top_indices, latent_1.top_indices
num_overlap, overlap_raio = count_common(top_0, top_1)
print(num_overlap, overlap_raio)
# num_iters = 20
# k = 2000
# num_adv = 1
# batch_size = 2000
