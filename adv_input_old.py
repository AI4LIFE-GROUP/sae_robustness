...
print(f"Initial overlap ratio = {count_common(top_idx_src, top_idx_target) / len(top_idx_src)}")
...
if attack_mode == 'suffix':
    x_src = torch.tensor(tokenizer.encode(src_text + ('* ' * num_adv))).unsqueeze(0).to(DEVICE)
elif attack_mode == 'prefix':
    x_src = torch.tensor(tokenizer.encode('*' + (' *' * (num_adv-1)) + src_text)).unsqueeze(0).to(DEVICE)
model.to(DEVICE)
best_loss = 100.0
similarities = []
overlaps = []
...
for i in range(num_iters):
    ...
    print(f"Iteration {i+1} num_overlap = {num_overlap}, overlap_ratio={num_overlap / len(top_idx_target)}")   
